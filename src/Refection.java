import javax.swing.*;
import java.lang.reflect.Field;
import java.util.List;

public class Refection {
    public static JTable getJTable(List<Object> lisatCliProd) {

        int i=0,j=0;
        int k=0;
        int numberOfFields = lisatCliProd.get(0).getClass().getDeclaredFields().length;
        Object [][] tabela = new Object[lisatCliProd.size()][numberOfFields];
        String [] listaCampuri = new String[numberOfFields];

            for (Field field : lisatCliProd.get(0).getClass().getDeclaredFields()) {

                field.setAccessible(true);
                 listaCampuri[k]=field.getName();
                k++;

            }

        for(Object it : lisatCliProd){
            for (Field field : it.getClass().getDeclaredFields()) {
                Object value;
                field.setAccessible(true);
               //listaCampuri[k]=field.getName();
                //System.out.println(field.getName());


                try {
                    value = field.get(it);
                    tabela[i][j]=value;

                } catch (IllegalAccessException e) {
                    e.printStackTrace();
                }


                j=j+1;
                System.out.println("");

            }
            j=0;
            i=i+1;
        }

        JTable t = new JTable(tabela,listaCampuri);
        return t;
    }
}
